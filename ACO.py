...
def apply_decay():
    for node in g_nodes_int:
        for adj_node in g_nodes_int:
            phero_matrix[node, adj_node] = phero_matrix[node, adj_node] * (
                1 - phero_decay
            )
...
def solve(input_graph, num_ants=10, iter=10, a=1, b=3, decay=0.8):
    ...
    for i in range(number_iterations):
        ...
        if final_costs == 0:
            ...
            iterations_needed = i + 1
        # print evolution
        print("Iteration: " + str(i + 1) + "  Best solution: " + str(final_costs))
...
def main():
    # create graph
    g = create_graph("gc_250_9")
    # solve
    final_costs, final_solution, iterations_needed = solve(
        g, num_ants=10, iter=50, a=1, b=3, decay=0.8
    )
    ...
    plt.show()
    print("Final costs: " + str(final_costs))
